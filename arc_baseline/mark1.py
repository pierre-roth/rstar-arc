...
def parse_args():
    ...
    parser.add_argument('--llm', type=str, default=DEFAULT_POLICY_LLM,
    ...
    parser.add_argument('--all-tasks', action='store_true', default=False,
                        help='Process all tasks in the data_sample directory')
...
try:
    ...
    # Convert input to grid using data_sample (safe in this controlled environment)
    grid = None
    if input_str:
        try:
            grid = data_sample(input_str)
        except Exception as e:
