...
def run_single_task(config: Config, task_path=None):
    ...
    if search_mode == "bs" or search_mode == "beam_search" or search_mode == "beam":
        logging.info(f"Using Beam Search mode")
        agent = BeamSearch(config)
    elif search_mode == "mcts" or search_mode == "monte_carlo" or search_mode == "monte-carlo":
        logging.info(f"Using Monte Carlo Tree Search mode")
        agent = MCTS(config)
    else:
        logging.error(f"Unknown search mode: {config.search_mode}")
        sys.exit(1)
...
def run_all_tasks(config: Config):
    ...
    logging.info(f"Processing all tasks in folder: {config.data_folder}")

    files = config.list_task_files()

    results = {}
    for i, file_name in enumerate(files):
        logging.info(f"Processing task {i + 1}/{len(files)}: {file_name}")
    ...
    logging.info(f"Overall results: {successful}/{total} tasks solved successfully ({successful / total * 100:.2f}%)")
