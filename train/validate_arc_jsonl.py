    ...
    for input_path in args.files:
        ...
        if args.overwrite:
            ...
            if not ext.lower() == ".jsonl":
                print(
                    f"Warning: Input file '{input_path}' does not have .jsonl extension. Output suffix might look odd.")
        ...
        if not args.overwrite and os.path.abspath(input_path) == os.path.abspath(output_path):
            print(
                f"Error: Input and output paths are identical ('{input_path}'). Use --overwrite or change --output-dir/--suffix.")
