...
def object_position(obj, background=Color.BLACK, anchor="upper left"):
    ...
    anchor = anchor.lower().replace(" ", "").replace("top", "upper").replace("bottom",
                                                                             "lower")  # robustness to mistakes by llm
...
def randomly_spaced_indices(max_len, n_indices, border_size=1, padding=1):
    ...
    """
    if border_size > 0:
        return randomly_spaced_indices(max_len - border_size - 2, n_indices, border_size=0,
                                       padding=padding) + border_size
...
def detect_translational_symmetry(grid, ignore_colors=[Color.BLACK], background=None):
    ...
    xy_possibilities = [TranslationalSymmetry(translate_x, translate_y) for translate_x in range(1, n) for translate_y
                        in range(1, m)]
...
def _score_symmetry(grid, symmetry, ignore_colors, background=None):
    ...
    if False and bad_mapping == 0:
        ...
        print("zero bad mapping, perfect ", perfect_mapping, "out of", n_occupied, "but this many off canvas",
              off_canvas, "using", symmetry)
        import pdb;
        pdb.set_trace()
...
def visualize(input_generator, transform, n_examples=5, n_attempts=100):
    ...
    if len(failures) > 0:
        print(
            f"{len(failures)}/{len(successes) + len(failures)} failures ({len(failures) / (len(failures) + len(successes))}). For example:")
        print(failures[0])
...
def random_sprite(n, m, density=0.5, symmetry=None, color_palette=None, connectivity=4, background=Color.BLACK):
    ...
    return random_sprite(n_original, m_original, density_original, symmetry_original, color_palette_original,
                         connectivity_original, background_original)


def detect_objects(grid, _=None, predicate=None, background=Color.BLACK, monochromatic=False, connectivity=None,
                   allowed_dimensions=None, colors=None, can_overlap=False):
    """
    ...
    if connectivity:
        objects.extend(find_connected_components(grid, background=background, connectivity=connectivity,
                                                 monochromatic=monochromatic))
        if colors:
            objects = [obj for obj in objects if
                       all((color in colors) or color == background for color in obj.flatten())]
        if predicate:
            ...
            for i in range(grid.shape[0] - n + 1):
                for j in range(grid.shape[1] - m + 1):
                    ...
                    if np.any(candidate_sprite != background) and \
                            (colors is None or all(
                                (color in colors) or color == background for color in candidate_sprite.flatten())) and \
    ...
    if not can_overlap:
        ...
        objects.sort(key=lambda obj: (crop(obj, background).shape[0] * crop(obj, background).shape[1],
        ...
        def pick_objects(remaining_indices, current_indices, current_mask):
            ...
            # Try picking it
            with_index, with_goodness = pick_objects(rest, current_indices + [first_index],
                                                     current_mask | (objects[first_index] != background))
