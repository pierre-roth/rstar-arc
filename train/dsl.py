...
def square(
    ...
    return len(piece) == len(piece[0]) if isinstance(piece, tuple) else height(piece) * width(piece) == len(
        piece) and height(piece) == width(piece)
...
def bordering(
    ...
    return uppermost(patch) == 0 or leftmost(patch) == 0 or lowermost(patch) == len(grid) - 1 or rightmost(
        patch) == len(grid[0]) - 1
...
def vupscale(
    ...
    upscaled_grid = tuple()
    for row in grid:
        upscaled_grid += tuple(row for num in range(factor))
    return upscaled_grid
...
def upscale(
    ...
    if isinstance(element, tuple):
        ...
        for row in element:
            ...
            for value in row:
                upscaled_row += tuple(value for num in range(factor))
            upscaled_grid += tuple(upscaled_row for num in range(factor))
        return upscaled_grid
    else:
        if len(element) == 0:
            return frozenset()
...
def downscale(
    ...
    for i in range(h):
        ...
        for j in range(w):
            if j % factor == 0:
                downscaled_row += grid[i][j],
        downscaled_grid += downscaled_row,
    h = len(downscaled_grid)
    downscaled_grid2 = tuple()
    for i in range(h):
        if i % factor == 0:
            downscaled_grid2 += downscaled_grid[i],
